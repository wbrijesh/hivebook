#!/usr/bin/env bash
# Idempotent ZITADEL provisioning: project "Trenches" + web (PKCE) and api apps,
# admin email, and the `trenches-oidc` secret consumed by the web build + Go API.
...
ISS="https://id.trenches.localhost"
...
echo ">> ensuring project 'Trenches'..."
PROJECT_ID=$(api -X POST "$MGMT/projects/_search" \
  -d '{"queries":[{"nameQuery":{"name":"Trenches","method":"TEXT_QUERY_METHOD_EQUALS"}}]}' \
  | jq -r '.result[0].id // empty')
[ -z "$PROJECT_ID" ] && PROJECT_ID=$(api -X POST "$MGMT/projects" -d '{"name":"Trenches"}' | jq -r '.id')
...
[ -z "$WEB_CLIENT_ID" ] && WEB_CLIENT_ID=$(api -X POST "$MGMT/projects/$PROJECT_ID/apps/oidc" -d '{
  "name":"web",
  "redirectUris":["https://app.trenches.localhost/callback"],
  "postLogoutRedirectUris":["https://app.trenches.localhost/"],
...
echo ">> setting admin email -> admin@trenches.localhost..."
...
[ -n "$ADMIN_ID" ] && api -X PUT "$MGMT/users/$ADMIN_ID/email" \
  -d '{"email":"admin@trenches.localhost","isEmailVerified":true}' >/dev/null
...
echo ">> writing trenches-oidc secret..."
kubectl get ns trenches >/dev/null 2>&1 || kubectl create ns trenches
kubectl -n trenches create secret generic trenches-oidc \
