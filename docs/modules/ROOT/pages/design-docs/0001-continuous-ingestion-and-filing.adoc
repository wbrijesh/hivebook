...
:supersedes:
:superseded-by:
:related-adrs: 0004, 0005, 0011

== Context

Hivebook ingests data continuously from many source systems and
...
a: "Raw ingestion\ncontinuous, no LLM"
b: "Segmentation\nper-source rules"
c: "Batched filing + structural decisions + entity resolution\none LLM call per batch"
d: "Apply plan\nfile · mutate structure · mark summaries stale"
e: Update metadata
a -> b -> c -> d -> e
----

Entity resolution rides inside the same batched LLM call. See
xref:ROOT:design-docs/0003-entity-reconciliation.adoc[] for the resolution-specific
mechanics.
...
[arabic]
. *Candidate sub-trees*: vector {plus} BM25 retrieval over the book
...
. *Chapter table of contents*: full list of chapter and topic
IDs and titles (no summaries). Cheap and bounded — even a
mature book stays in a low-thousands token range.
. *Recent entity context*: container priors and the recent
resolution sliding window for the container (see xref:ROOT:design-docs/0003-entity-reconciliation.adoc[0003]).
. *Entity store snapshot*: canonical names and aliases for the
...
* Chapter ToC (titles {plus} IDs only)
...
[arabic]
. Each filing with confidence ≥ `HIGH++_++CONFIDENCE` (tunable
...
chapter ToC only (titles {plus} IDs), with full detail only for
...
* Exact value of `HIGH++_++CONFIDENCE` for direct apply — starts at
...
* Batched LLM filing as the filing strategy — xref:ROOT:adrs/0005-batched-llm-filing.adoc[ADR-0005].
* Continuous structure design from a generic chapter skeleton, no
per-customer bootstrap — xref:ROOT:adrs/0004-continuous-structure-design.adoc[ADR-0004].
* 15 min / 20 items batch trigger as a tunable default — xref:ROOT:adrs/0011-constants-first-tunables.adoc[ADR-0011].
