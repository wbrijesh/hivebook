...
____
*Apex* is the Hivebook design system. Every UI decision should be
checkable against this document.
____

== North star

*Hivebook should feel engineered, not marketed — a precise, quiet instrument
...
They share exactly one aesthetic: *restraint.* The veteran reads restraint {plus}
correctness as _rigor and respect_; the founder reads the same restraint as
_premium and stable_. So we don’t compromise between them — we serve both by
...
[arabic]
. *Subtract first.* The default answer to "`should we add this?`" is no.
Decoration is suspect; every element earns its place or leaves.
. *Correctness is the aesthetic.* The small things being exactly right —
the email that follows you, the provider name that matches the button, code
vs. link — _is_ the design. It’s what the veteran admires and the founder
feels as stability.
. *Quiet over friendly.* Confidence is shown by saying less, not by being
warm. We respect the user’s intelligence.
. *Content is the hero.* Chrome recedes; the corpus and the user’s data are
what’s bright. Color and motion are rationed.
. *Predictable over delightful.* No surprises, no easter eggs, no
performative motion. It behaves exactly as a competent user expects.
. *Built for people who live in tools.* Keyboard-respecting, legible state,
power affordances present but never required.
. *One system, not many screens.* Decisions are made once, in tokens and
...
* Don’t name the product in UI copy. The reader knows what app they’re in —
  repeating "`Hivebook`" through labels and descriptions reads as marketing, not
  product. Say "`sync`", not "`Hivebook syncs`". Reserve the name for first-touch
  surfaces (sign-in, the logo, page metadata), never sprinkled through everyday copy.
...
|"`Choose which projects Hivebook should sync.`" |"`Choose which projects to sync.`"
|===

== Visual system

Tokens live in `web/app/globals.css` (the prototype carries its own copy). Never
use raw Tailwind palette classes; always the semantic tokens. The token file is
also the single brand source (xref:ROOT:design-docs/0007-configuration-pattern.adoc[design-doc 0007]).

=== Color

* *Neutrals:* pure Adobe Spectrum gray (no hue tint).
* *Accent:* one hue, used sparingly — primary buttons, focus rings, links,
selected state. Carries meaning; never decorative. *Cyan* (Adobe Spectrum
cyan-900), fixed; change it in `web/app/globals.css`. (The live accent/font
picker is a `prototype/`-only dev tool, not shipped in the app.)
* *Status:* `success` (green), `warning` (yellow — kept distinct from the
accent), `destructive` (red). Each has a `++*++-subtle` background {plus} foreground
pair for badges and callouts.
* *Surfaces — two content planes {plus} one frame plane, identical logic in light
and dark:*
...
_both_ themes — the frame reads the same whether the app is light or dark,
...
*application* is the opposite register: it must read as a _system_ — software
...
[arabic]
. *Frame, don’t header.* Global navigation is the *spine* (a dark icon
rail of surfaces {plus} workspace {plus} search {plus} profile), always present — not a
...
. *Every object wears its state.* A row is never just a title and a chevron.
...
. *Show the machinery.* Ingestion (sources → artifacts), the build/rebuild
...
. *Multi-pane over single-column.* Reading and admin surfaces pair a content
...
. *Density is the dense end of the type scale.* 13px workhorse, 12px
...
. *No silent emptiness.* A surface with nothing to show still states the
system fact ("`building`", "`0 sources — they appear as the summary is built`"),
never a blank panel.

This is restraint _applied to information_, not restraint _as the absence of
it_: we show what serves the operator and nothing that merely decorates. The
