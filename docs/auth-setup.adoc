...
web/api apps, sets the admin email, and writes the `trenches-oidc` Secret, or
...
result is four values dropped into the `trenches-oidc` Secret (template:
...
|===
|Key |From |Used by
|`OIDC++_++ISSUER` |`https://id.trenches.localhost` |web build, api
...
* `POST /projects` `++{++"name":"Trenches"}` → `PROJECT++_++ID`
