...
hierarchy invariant from `corpus-and-index.adoc`: sub-topic summaries
...
[arabic]
. Gather the citations. Sub-topic builds gather source spans;
topic and chapter builds gather child summaries.
. Hand to the LLM with a structured prompt: produce a summary,
cite every claim, flag contradictions or unknowns.
. Validate that every claim has at least one citation. Reject and
retry if not.
. Store the new summary with `last++_++rebuilt = now`, citations, and
