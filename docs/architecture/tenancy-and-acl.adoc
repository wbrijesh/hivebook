...
Hivebook enforces access at two levels: across customers (strict
...
Hivebook does *not* inherit source-system ACLs and does *not*
...
layer (see `enterprise-controls.adoc`). There is no per-source
identity mapping in Hivebook — a consequence of not using source
ACLs.
...
choose. See `enterprise-controls.adoc`.

=== Audit

Every read and every privileged write is recorded to a per-tenant
audit log. See `enterprise-controls.adoc` for the audit log
specification.
