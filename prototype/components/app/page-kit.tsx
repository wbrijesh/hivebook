    ...
    <div className="mt-4 flex flex-wrap divide-x divide-border overflow-hidden rounded-md border border-border bg-card">
      {items.map((s) => (
        <div key={s.label} className="min-w-[136px] flex-1 px-4 py-2.5">
          ...
          <div className="mt-0.5 text-[18px] font-semibold leading-none tabular-nums text-foreground">
            {s.value}
          </div>
          {s.sub && <div className="mt-1 text-[12px] text-muted-foreground">{s.sub}</div>}
        </div>
      ))}
    </div>
  )
...
export function Num({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        "shrink-0 text-right font-mono text-[12px] tabular-nums text-foreground",
        className
      )}
    >
      {children}
...
export function Dot({ tone, pulse }: { tone: Tone; pulse?: boolean }) {
  return (
    <span
      className={cn("size-1.5 shrink-0 rounded-full", TONE_DOT[tone], pulse && "animate-pulse")}
    />
  )
}
